...
class MeterListAPI(Resource):
    def get(self):
        return jsonify({'meterlist':'GET'})

    def post(self):
        return jsonify({'meterlist': 'POST'})

class MeterAPI(Resource):
    def get(self, id):
        return jsonify({'meter': str(id)})
...
""" Example of how to upload a file using flask-restful """
